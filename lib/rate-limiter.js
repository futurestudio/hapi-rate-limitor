...
class RateLimiter {
  ...
  constructor (server, options = {}) {
    ...
    this.emitter = new RateLimitEventEmitter({ server, emitter })
  }
  ...
  async handle (request, h) {
    ...
    if (request.rateLimit.isInQuota()) {
      await this.emitter.fireInQuotaEvent(request)

      return h.continue
    }

    await this.emitter.fireExceededEvent(request)
   ...
   * @returns {Boolean}
   */
  hasMaxAttemptsRouteConfig (request) {
  ...
  /**
   * Determine the rate limit of the given `request`.
   *
   * @param {Request} request
   *
  ...
  async rateLimit (request) {
    await this.emitter.fireAttemptEvent(request)
  ...
  async requestIdentifier (request) {
    return this.hasMaxAttemptsRouteConfig(request)
