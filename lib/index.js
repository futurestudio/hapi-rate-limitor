 ...
 * @returns {Object}
 */
function getFromRequest (request, userIdKey, userLimitKey) {
  if (shouldLimitForUser(request, userIdKey, userLimitKey)) {
    return {
      id: request.auth.credentials[userIdKey],
      limit: request.auth.credentials[userLimitKey]
    }
  }
...
async function register (server, options = {}) {
  ...
  server.ext('onPostAuth', async (request, h) => {
    const { id, limit } = getFromRequest(request, userIdKey, userLimitKey)
