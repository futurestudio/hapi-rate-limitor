 ...
 * @returns {Integer}
 */
function getUserLimit (request, userLimitKey) {
...
async function register (server, options) {
  ...
  /**
   * Extend the request lifecylce and check if the request
   * exceeds the rate limit.
   */
  server.ext('onPostAuth', async (request, h) => {
    const userLimit = getUserLimit(request, options.userLimitKey)
    const clientIP = detectClientIP(request)

    const routeConfig = request.route.settings.plugins['hapi-rate-limitor'] || {}
    const rateLimitConfig = Object.assign({ id: clientIP, max: userLimit }, routeConfig)

    request.rateLimit = await rateLimiter.get(rateLimitConfig)
